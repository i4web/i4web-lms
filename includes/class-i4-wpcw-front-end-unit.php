<?php
/**
 * I4Web_LMS Unit Front End Class. This handles displaying the Unit Action Buttons and messages how we want to
 * Based off of the WPCW_UnitFrontend class. Unable to extend the class, we just implemented our own variation.
 *
 * @package I4Web_LMS
 ...
 * @since 0.0.1
 */
class I4Web_LMS_Front_End_Unit {
    /**
    ...
    /**
     * Returns the HTML for the Unit Actions area
     *
     * @since 0.0.1
     */
    function i4_lms_unit_actions() {
        global $post;
        ...
        if (!$this->check_user_canUserAccessCourse()) {
            return $this->message_user_cannotAccessCourse();
        }

        // Check if the user has access to the unit
        if (!$this->check_user_canUserAccessUnit()) {
            return $this->message_user_cannotAccessUnit();
        }

        //Display Actions or Messages based on the the status of this unit
        $html = $this->render_detailsForUnit();

        //  $html = '<div class="i4-unit-actions-wrapper"></div>';
     ...
     *
     * @return String HTML to render for the unit.
     */
    function render_detailsForUnit($justGetCompletionAndQuizData = false) {
        $completionBox = false;
        $extraMessages = false;
        $quizProgressData = false;

        // ####�Timers - Check if the timer has timed out for the quiz.
        ...
        if ($justGetCompletionAndQuizData) {
            ...
            $unit_details_html = '<div id="wpcw_fe_outer_wrap">' . $completionBox . $extraMessages . $quizProgressData . '</div>' . $navigationBox;
            ...
            foreach ($this->unitQuizDetails->questions as $question) {
                ...
                if ($setting_showExplanation && $question->question_answer_explanation) {
                    $html .= sprintf('<div class="wpcw_fe_quiz_q_explanation"><b>%s:</b>&nbsp;&nbsp;%s</div>',
                        ...
                        nl2br($question->question_answer_explanation)
                    );
                }
        ...
        if ($this->check_quizzes_NonBlockingQuizOffersRetakeButton()) {
            ...
            if ($showRetakeButton) {
                $messageToShow .= sprintf('<div class="wpcw_fe_quiz_retake">
  				...
  				<a href="#" class="fe_btn fe_btn_completion btn_completion button blue" data-wpcw_quiz="%d" data-wpcw_unit="%d">%s</a>
  			</div>',
                ...
                $html .= I4Web_LMS_Front_End_Unit::message_createMessage_warning($messageToShow);
            }
    ...
    function render_completionBox_quizPassStatus() {
        ...
        if (isset($msgDetails['button_dl_results_url'])) {
            $messageToShow .= sprintf('
  				<div class="wpcw_fe_progress_download">
  					<a href="%s" class="fe_btn fe_btn_completion fe_btn_small button blue">%s</a>
  				</div>',
        ...
        if ($messageToShow) {
            if ($msgDetails['error_mode']) {
                $html .= I4Web_LMS_Front_End_Unit::message_createMessage_error($messageToShow);
            }
            else {
                $html .= I4Web_LMS_Front_End_Unit::message_createMessage_success($messageToShow);
            }
        }
            ...
            if ($this->check_quizzes_hasUserCompletedQuiz()) {
                    ...
                    if (
                        ...
                        // less than the permitted count for the quiz. This filters out the state of quiz waiting too.
                            ...
                            // No retakes left, so show a suitable message.
                                ...
                                $showRetakeButton = false;
                                $messageToShow .= wpautop(__('Unfortunately you have reached the maximum limit of attempts you are permitted for this quiz.', 'wp_courseware'));
                                //$messageToShow .= wpautop(__('You have reached the maximum limit of attempts, however your instructor has allowed another retake.', 'wp_courseware'));
                                if ($this->unitQuizProgress->quiz_next_step_msg) {
                                    $messageToShow .= wpautop(stripslashes($this->unitQuizProgress->quiz_next_step_msg));
                                }
                            }
                ...
                if ($this->unitQuizProgress->quiz_needs_marking > 0) {
                    ...
                    if ('quiz_block' == $this->unitQuizDetails->quiz_type) {
                        $html .= I4Web_LMS_Front_End_Unit::message_createMessage_success($this->parentData->course_message_quiz_open_grading_blocking);
                    }
                }
            }
    ...
    function render_quizzes_handleQuizRendering($showErrorsOnForm = true) {
        ...
        if ($this->check_paging_shouldWeShowReviewPage_rightNow()) {
            ...
            $html .= I4Web_LMS_Front_End_Unit::message_createMessage_warning(__('You can now review your answers before submitting them.', 'wp_courseware'));
        }
                ...
                if (empty($this->unitQuizProgress) || 'retake_waiting' == $this->unitQuizProgress->quiz_next_step_type) {
                    ...
                    $html .= sprintf('
  									<div class="wpcw_fe_quiz_begin_quiz">
  										...
  										<a href="#" class="fe_btn fe_btn_completion button blue" id="wpcw_fe_quiz_begin_quiz" data-wpcw_quiz="%d" data-wpcw_unit="%d" >%s</a>
            ...
            if ($showQuestions) {
                ...
                if ($this->check_paging_areWePagingQuestions()) {
                    ...
                    if ($this->check_paging_shouldWeShowAnswerLaterButton()) {
                        $html .= sprintf('<div class="wpcw_fe_quiz_answer_later"><a href="#" class="fe_btn fe_btn_small fe_btn_navigation button" id="wpcw_fe_quiz_answer_later">%s</a></div>',
                    ...
                    if ($this->check_paging_shouldWeShowPreviousButton()) {
                        $buttonPreviousClicker = sprintf('<input type="submit" class="fe_btn fe_btn_completion btn_completion button blue" id="fe_btn_quiz_previous" name="previous_question" value="%s">',
                    ...
                    $html .= sprintf('<div class="wpcw_fe_quiz_submit wpcw_fe_quiz_submit_data">
  										...
  										%s
  										<input type="submit" class="fe_btn fe_btn_completion btn_completion button blue" id="fe_btn_quiz_next" name="submit" value="%s">
  									</div>',
                ...
                // Are we showing the form with all questions?
                    ...
                    $html .= sprintf('<div class="wpcw_fe_quiz_submit wpcw_fe_quiz_submit_data">
  										...
  										<input type="submit" class="fe_btn fe_btn_completion btn_completion button blue" name="submit" value="%s">
  									</div>',
    ...
    function render_navigation_getNavigationBox($disableNextButton = false) {
        ...
        $html .= '<h5 class="course-nav-title">Course Navigation</h4>';
    ...
    function fetch_quizzes_questionResultsByTag() {
        ...
        foreach ($this->unitQuizDetails->questions as $questionID => $questionObj) {
            ...
            if (!empty($questionObj->tags)) {
                ...
                // ###�5 - Work out if an open-ended question or not.
    ...
    static function message_error_getCompletionBox_error() {
        return I4Web_LMS_Front_End_Unit::message_createMessage_error(__('An error occured, so your unit progress was not updated. Please refresh the page and try again.', 'wp_courseware'));
    }
    ...
    function message_user_notLoggedIn() {
        ...
        $newContent = I4Web_LMS_Front_End_Unit::message_createMessage_error($this->parentData->course_message_unit_not_logged_in);
    ...
    function message_user_cannotAccessCourse() {
        // Return no access error without content
        $newContent = I4Web_LMS_Front_End_Unit::message_createMessage_error($this->parentData->course_message_unit_no_access);
    ...
    function message_user_cannotAccessUnit() {
        // Return no access error without content
        $newContent = I4Web_LMS_Front_End_Unit::message_createMessage_error($this->parentData->course_message_unit_not_yet);
    ...
    function check_quizzes_canWeContinue_checkAnswersFromOnePageQuiz($potentialAnswers) {
        ...
        if ($this->unitQuizDetails->questions && $answerCountSoFar < count($this->unitQuizDetails->questions)) {
            ...
            echo I4Web_LMS_Front_End_Unit::message_createMessage_error(__('Please provide an answer for all of the questions on this page.', 'wp_courseware'));
    ...
    function check_quizzes_gradeQuestionsForQuiz() {
        ...
        if ('survey' == $this->unitQuizDetails->quiz_type) {
            ...
            echo I4Web_LMS_Front_End_Unit::message_createMessage_success(__('Thank you for your responses. This unit is now complete.', 'wp_courseware'));
        }
        ...
        // We should answers for all questions by this point.
            ...
            // #### Step B - we have no-open-ended questions, just T/F or Multiple-choice
                ...
                if ('quiz_noblock' == $this->unitQuizDetails->quiz_type) {
                    ...
                    /*
                    echo I4Web_LMS_Front_End_Unit::message_createMessage_success(sprintf(__('You got <b>%d out of %d (%d%%)</b> questions correct! This unit is now complete.', 'wp_courseware'),
                ...
                // Blocking quiz (quiz_type == quiz_block).
                    ...
                    // They've failed. Report which questions are correct or incorrect.
                        ...
                        /*echo I4Web_LMS_Front_End_Unit::message_createMessage_error(
    ...
    function fetch_paging_getQuestion_moveQuestionMarker($jumpMode) {
        ...
        if (!empty($this->unitQuizProgress)) {
            ...
            // Moving to previous question - Only attempt to move the marker if it's already > 0.
                ...
                if ($this->unitQuizProgress->quiz_paging_next_q > 0) {
                    $newIndex--;
                }
            }
